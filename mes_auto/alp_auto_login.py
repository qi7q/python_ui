...
def save_cookies():
    ...
    cookies_file=file_path+"alp.cookies"
...
def login():
    ...
    try:
        #进入RTM登录页面
        driver.get("http://10.16.12.167:8080/Account/Login?ReturnUrl=%2f")
        time.sleep(2)

        driver.find_element_by_id("userinput").send_keys("yanqc2")
        ...
        driver.get("http://10.16.12.167:8080/zh-CN/MES/SfcWorkCenter")
        time.sleep(3)
        save_cookies()
    finally:
        time.sleep(3)
...
def get_login_with_cookies():
    ...
    #验证是否登录成功
    login_success_url="http://10.16.12.167:8080/zh-CN/MES/SfcWorkCenter"
...
def screenshot(driver,path=None):
    ...
    if path==None:
        driver.save_screenshot(file_path+"ALP-"+image_name)
